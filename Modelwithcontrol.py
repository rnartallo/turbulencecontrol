...
def HopfPhaseModelControlled(phi,t,p):
    N,w,G,delta_x,alpha,D,h,mu = p
    dOsc = np.zeros(N)
    ...
    R_global,theta_global = CalculateGlobalKuramotoOrderParam(phi[:,time_step])
    for n in range(0,N):
        dOsc[n] = w-R_dis[n]*np.sin(phi[n,time_step]-Theta_dis[n]-alpha) + mu*R_global*np.sin(theta_global-phi[n,time_step])
    return dOsc

def SolvePhaseHopfModelControlled(phi_0,T,N,w,G,num_points,t_0,delta_x,alpha,D,mu):
    t = np.linspace(t_0,T,num_points)
    phi = np.zeros((N,num_points))
    phi[:,0]=phi_0
    h=(T-t_0)/num_points
    p=[N,w,G,delta_x,alpha,D,h,mu]
    noise=np.random.normal(0,np.sqrt(h),(N,num_points))
    wiener = np.cumsum(noise,axis=1)
    for n in range(0,num_points-1):
        phi[:,n+1] = phi[:,n]+ h*HopfPhaseModelControlled(phi,t[n],p) + np.sqrt(D)*h*(wiener[:,n+1]-wiener[:,n])
    return [phi,t]
