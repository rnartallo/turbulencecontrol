...
def ComplexHopfModel(W,t,p):
    N,G,delta_x,alpha,K,h,beta,w = p
...
def SolveComplexHopfModel(W_0,T,N,w,G,num_points,t_0,delta_x,alpha,sigma,K,beta):
    t = np.linspace(t_0,T,num_points)
    W = np.zeros((N,num_points),dtype=complex)
    W[:,0]=W_0
    h=(T-t_0)/num_points
    p=[N,G,delta_x,alpha,K,h,beta,w]
    noise=np.random.normal(0,np.sqrt(h),(N,num_points))
    wiener = np.cumsum(noise,axis=1)
    for n in range(0,num_points-1):
        W[:,n+1] = W[:,n]+ h*ComplexHopfModel(W,t[n],p) + np.sqrt(sigma)*h*(wiener[:,n+1]-wiener[:,n])
    return [W,t]